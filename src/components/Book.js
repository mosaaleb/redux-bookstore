...
const Book = ({ removeBook, book }) => {
  const [isUpdateProgressHidden, setIsUpdateProgressHidden] = useState(true);
  return (
    <div className="bg-white my-3 shadow p-3 rounded flex flex-col sm:flex-row sm:justify-between sm:p-8">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex-grow">
          {isUpdateProgressHidden ? (
            <button
              type="button"
              onClick={() => setIsUpdateProgressHidden(false)}
              className="text-sm px-3 py-2 uppercase bg-blue-600 text-gray-100 rounded focus:outline-none"
            >
              Update Progress
            </button>
          ) : (
            <UpdateProgressForm
              ...
              setIsUpdateProgressHidden={setIsUpdateProgressHidden}
            />
          )}
        </div>
      </div>
